...
class NewsMessenger:
    ...
    def send_report(self, report_path, report_data):
        print("📮 [메신저 v3] 디자인 개선 리포트 이메일 발송 준비...")
        
        if not self.email_user or not self.email_password:
            print("⚠️ [메신저 v3] 이메일 설정(GMAIL_USER, GMAIL_APP_PASSWORD)이 없습니다. 발송을 건너뜁니다.")
            return
        try:
            ...
            def score_value(news):
                try:
                    return int(news.get('score', 0) or 0)
                except Exception:
                    return 0

            def pick_signal(items):
                if not items:
                    return None
                category_rank = {
                    "물가 및 원재료": 5,
                ...
                return max(
                    ...
                    key=lambda n: (
                        ...
                        score_value(n),
                        category_rank.get(str(n.get('category', '미분류')).strip(), 0),
                    )
                )

            categorized_news = {cat: [] for cat in CATEGORY_ORDER}
            for news in report_data:
                cat = str(news.get('category', '미분류')).strip()
                if cat not in CATEGORY_ORDER:
                    cat = '미분류'
                categorized_news[cat].append(news)
            ...
            issue_no = datetime.now().isocalendar()[1]
            critical_count = sum(1 for n in report_data if n.get('is_critical'))
            signal_news = pick_signal(report_data)

            # 헤더 이미지 로드 및 CID 구성
            header_image_cid = "jinju_header_v3"
            ...
            if has_header_img:
                brand_banner_html = f"""
                <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top: 24px; margin-bottom: 14px;">
                    <tr>
                        <td align="center">
                            <img src="cid:{header_image_cid}" alt="대한민국 최초의 육가공 기업 진주햄" width="636" style="width: 100%; max-width: 636px; height: auto; display: block; border: 0; border-radius: 14px;" />
                        </td>
                    </tr>
                </table>
                """
                ...
                brand_banner_html = """
                <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top: 24px; margin-bottom: 14px; background-color: #0E2D2A; border-radius: 14px;">
                    <tr><td align="center" style="padding: 28px 16px; color: #FFFFFF; font-size: 24px; font-weight: 900; letter-spacing: -0.5px;">대한민국 최초의 육가공 기업, 진주햄</td></tr>
                </table>
                """
            ...
            for category in CATEGORY_ORDER:
                ...
                chips_html += f'<span style="display:inline-block; margin:0 6px 8px 0; padding:6px 10px; border:1px solid #DCE8E1; border-radius:18px; background-color:#FFFFFF; color:{color}; font-size:12px; font-weight:800;">{emoji} {esc(category)} {count}</span>'

            if signal_news:
                signal_title = esc(signal_news.get('title'))
                signal_link = esc(signal_news.get('link'))
                signal_insight = esc(signal_news.get('insight') or signal_news.get('desc') or '')
                signal_cat = str(signal_news.get('category', '미분류')).strip()
                signal_color = CATEGORY_COLORS.get(signal_cat, "#17B890")
                signal_html = f"""
                <table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#132017" style="margin: 0 0 22px 0; background-color: #132017; border-radius: 18px;">
                    <tr>
                        <td style="padding: 18px 18px 18px 20px; vertical-align: top;">
                            <div style="font-size: 10px; color: #B9DFC7; font-weight: 900; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 7px;">TODAY'S SIGNAL · {esc(signal_cat)}</div>
                            <a href="{signal_link}" style="color: #FFFFFF; text-decoration: none; font-size: 19px; line-height: 1.35; font-weight: 900;">{signal_title}</a>
                            <p style="margin: 9px 0 0 0; color: #E4EEE8; font-size: 13px; line-height: 1.65;">{signal_insight}</p>
                        </td>
                        <td width="110" align="center" style="padding: 18px 16px 18px 0; vertical-align: middle;">
                            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.16); border-radius: 14px;">
                                <tr><td align="center" style="padding: 12px 6px;"><div style="font-size: 28px; line-height: 1; color: #FFFFFF; font-weight: 900;">{critical_count}</div><div style="font-size: 11px; color: #D7E7DE; font-weight: 800; margin-top: 4px;">주의 신호</div></td></tr>
                            </table>
                        </td>
                    </tr>
                    <tr><td colspan="2" style="height: 4px; background-color: {signal_color}; font-size: 0; line-height: 0; border-radius: 0 0 18px 18px;">&nbsp;</td></tr>
                </table>
                """
            else:
                signal_html = ""
            ...
            for category in CATEGORY_ORDER:
                ...
                <tr><td style="padding: 20px 0 8px 0;">
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                        <tr>
                            <td style="border-top: 3px solid {color}; padding-top: 7px; vertical-align: top; width: 84px;">
                                <span style="background-color: {color}; color: #fff; font-size: 9px; font-weight: 900; letter-spacing: 1px; padding: 3px 7px; white-space: nowrap; display: inline-block; border-radius: 10px;">{label}</span>
                            </td>
                            <td style="border-top: 1px solid #DDE8E1; padding-top: 6px; vertical-align: top;">
                                <span style="color: #15231B; font-size: 17px; font-weight: 900; letter-spacing: -0.2px;">{emoji} {esc(category)}</span>
                                <span style="color: #7D8D84; font-size: 12px; margin-left: 8px;">{len(items)}건</span>
                            </td>
                        </tr>
                    </table>
                </td></tr>
                ...
                for news in items:
                    article_no += 1
                    title = esc(news.get('title'))
                    link = esc(news.get('link'))
                    insight = esc(news.get('insight') or news.get('desc') or '')
                    keywords = esc(news.get('reasons') or '')
                    score = score_value(news)
                    urgent_badge = '<span style="display:inline-block; margin-left:6px; padding:2px 6px; border-radius:8px; background-color:#FBE7E3; color:#B53A2B; font-size:10px; font-weight:900;">주의</span>' if news.get('is_critical') else ''

                    news_items_html += f"""
                    <tr><td style="padding: 7px 0;">
                        <table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#FFFFFF" style="background-color: #FFFFFF; border: 1px solid #E1E9E4; border-left: 4px solid {color}; border-radius: 14px;">
                            <tr>
                                <td width="44" align="center" style="padding: 14px 8px; vertical-align: top; color: {color}; font-size: 13px; font-weight: 900; font-family: Georgia, 'Times New Roman', serif;">{article_no:02d}</td>
                                <td style="padding: 13px 14px 13px 0; vertical-align: top;">
                                    <div style="margin-bottom: 7px;"><a href="{link}" style="color: #101828; text-decoration: none; font-size: 16px; font-weight: 900; line-height: 1.38;">{title}</a>{urgent_badge}</div>
                                    <p style="margin: 0; color: #435065; font-size: 13px; line-height: 1.65;"><span style="display: inline-block; margin-right: 6px; padding: 1px 6px; background-color: #F1F5F3; border-radius: 7px; color: #0F172A; font-size: 10px; font-weight: 900; letter-spacing: 0.5px;">SO WHAT</span>{insight}</p>
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top: 10px;"><tr>
                                        <td style="color: #728078; font-size: 11px; font-weight: 700; line-height: 1.4;">{keywords}</td>
                                        <td align="right" width="72" style="color: #728078; font-size: 11px; font-weight: 800; white-space: nowrap;">점수 {score}</td>
                                    </tr></table>
                                </td>
                            </tr>
                        </table>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#E8F0EC" style="width: 100%; margin: 0; padding: 30px 0; background-color: #E8F0EC;">
                <tr>
                    <td align="center" style="padding: 20px 0;">
                        <!--[if (gte mso 9)|(IE)]>
                        <table width="700" align="center" cellpadding="0" cellspacing="0" border="0"><tr><td align="center">
                        <![endif]-->
                        <table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#FBFCFB" style="max-width: 700px; width: 100%; margin: 0 auto; background-color: #FBFCFB; border: 1px solid #D7E5DD; border-radius: 22px;">
                            <tr>
                                <td style="padding: 0 32px 18px 32px; background-color: #FFFFFF;">
                                    {brand_banner_html}
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top: 2px solid #132017; border-bottom: 1px solid #132017;">
                                        <tr><td align="center" style="padding: 7px 0; font-size: 10px; font-weight: 800; letter-spacing: 3px; color: #4D5A51;">JINJU HAM &nbsp;&middot;&nbsp; FOOD INDUSTRY INTELLIGENCE &nbsp;&middot;&nbsp; INTERNAL USE ONLY</td></tr>
                                    </table>
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin: 16px 0 8px 0;">
                                        <tr><td align="center"><h1 style="margin: 0; font-size: 38px; font-weight: 900; letter-spacing: -1px; color: #101611; font-family: Georgia, 'Times New Roman', serif; line-height: 1;">식품 뉴스 클리핑</h1><p style="margin: 6px 0 0 0; font-size: 12px; color: #68756C;">V3 Design Preview — So-What First Briefing</p></td></tr>
                                    </table>
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top: 2px solid #111; border-bottom: 2px solid #111; margin-top: 14px;">
                                        <tr>
                                            <td align="left" style="padding: 7px 0; font-size: 11px; font-weight: 800; color: #28332C; width: 33%;">VOL. {datetime.now().year} &nbsp;|&nbsp; ISSUE {issue_no}</td>
                                            <td align="center" style="padding: 7px 0; font-size: 11px; color: #28332C; font-weight: 800; width: 34%;">선별 기사 {total_articles}건</td>
                                            <td align="right" style="padding: 7px 0; font-size: 11px; font-weight: 800; color: #28332C; width: 33%;">{today_formatted}</td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>
                            <tr><td style="padding: 22px 32px 0 32px;">{signal_html}</td></tr>
                            <tr><td style="padding: 0 32px 12px 32px;">{chips_html}</td></tr>
                            <tr>
                            ...
                            <tr>
                                <td style="padding: 0 32px 24px 32px;">
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top: 3px double #132017;">
                                        <tr>
                                            <td align="center" style="padding-top: 14px;">
                                                <p style="margin: 0 0 3px 0; font-size: 10px; color: #7D8D84; letter-spacing: 0.5px;">V3 design preview · Automatically curated by the AI Strategic Management System &middot; Powered by Google Gemini</p>
                                                <p style="margin: 0; font-size: 10px; color: #7D8D84;">&copy; {datetime.now().year} Jinju Ham Co., Ltd. Marketing &amp; Sales Division &nbsp;&middot;&nbsp; Strictly Confidential</p>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
            ...
            with smtplib.SMTP(self.smtp_server, self.smtp_port) as server:
                ...
                for recipient in self.recipients:
                    try:
                        ...
                        msg['To'] = recipient
                        msg['Subject'] = f"[V3 디자인 개선안] 식품업계 비즈니스 브리핑 ({datetime.now().strftime('%m/%d')})"
                        ...
                        if has_header_img:
                            try:
                                ...
                                print(f"⚠️ [메신저 v3] 헤더 이미지 CID 첨부 에러: {img_err}")
                        
                        server.send_message(msg)
                        print(f"✅ [메신저 v3] 이메일 발송 완료: {recipient}")
                    except Exception as e:
                        print(f"❌ [메신저 v3] {recipient} 발송 실패: {e}")
            
        except Exception as e:
            print(f"❌ [메신저 v3] 이메일 시스템 오류: {e}")
...
class NewsAgentSystem:
    ...
    def run(self):
        print("🚀 [System] News Agent Version 3.0 (Design Preview · So-What First) Loaded")
